...
void main(void)
{
	...
	// Position calculée dans l’espace de la caméra, puis projetée dans l’espace de la lumière.
	vec4 wlightSpacePosition = LigthToShadowMap * vec4(position, 1.0);
	vec3 lightSpacePosition = vec3(wlightSpacePosition/wlightSpacePosition.w);

	// Position calculée dans l'espace de la lumière
	vec4 shadowPixel = vec4(texture(ShadowMap, lightSpacePosition.xy).r, 0, 0, 1);
