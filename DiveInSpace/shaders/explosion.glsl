...
// Data 
//
vec3 iResolution = vec3(1024, 768, 0);
vec3 iChannelResolution = vec3(1024, 768, 0);
vec4 iMouse = vec4(100, 100, 10, 0);
...
vec4 raymarch( in vec3 rayo, in vec3 rayd, in float expInter ){
	...
	vec3 pos = rayo + rayd * (expInter + step*texture2D( Channel, gl_FragCoord.xy/iChannelResolution.x ).x);
...
void main(void)
{
    ...
    Color = vec4(col.xyz*col.xyz*(3.0-2.0*col.xyz), 1);
}
