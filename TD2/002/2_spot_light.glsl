...
vec3 computeSpotLight(float intensity, float specCoeff, vec3 diffuse, vec3 lPosition, vec3 position, vec3 normal, vec4 material) {
	...
	vec3 colorRes = lightColor * lightIntensity * fallOff * (diffuse * n_dot_l + specCoeff * vec3(1.0, 1.0, 1.0) *  pow(n_dot_h, specCoeff * LightSpecCoeff));
	return colorRes;
}
...
void main(void)
{
	...
	vec3 light = computeSpotLight(LightIntensity, specularCoeff, diffuse, LightPosition, position, normal, material);
	Color = vec4(light, 1.);
}
